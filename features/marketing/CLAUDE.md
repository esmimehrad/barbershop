...
One component per landing section, composed by `app/(marketing)/page.tsx`. Server Components by default; a handful of files are `'use client'` for real interactivity (`Hero`, `HeroDesktop`, `MobileMenuToggle`, `Preloader`, `BeforeAfterSlider`, `SmoothScrollProvider`, `CustomCursor`). Primary app navigation (Home/Book/Account) is the shared `components/ui/bottom-tab-bar.tsx`, mounted by the layout — the landing's old scroll-triggered `MobileBookBar` was retired in favor of its always-visible center "Book" FAB.
...
- **Hero — split by viewport into separate files so editing one never affects the other** (they share only the `HeroContent` copy):
  - `Hero.tsx` — thin selector, the ONLY place that decides mobile vs desktop. Reads `(max-width: 767px)` and re-checks on resize; before mount / no-JS it renders `StaticHero`.
  - `HeroMobile.tsx` — **mobile (< 768px)**, a single static image (`public/images/hero-final.jpg`) with a brightened bottom gradient. No video/scroll animation.
  - `HeroDesktop.tsx` — **desktop (≥ 768px)**, the original cinematic scroll-scrubbed 120-frame sequence (GSAP ScrollTrigger drives a `<canvas>`, pinned via plain CSS `position: sticky`, not GSAP's pin plugin; frames extracted from a source video the user supplied — see `public/images/README.md`). First-paints as `StaticHero` (single `hero-final.jpg`), which is also the reduced-motion / no-JS and SSR fallback (exported for `Hero.tsx`). `HeroIntro.tsx` is this cinematic's centered booking prompt.
  - `HeroContent.tsx` — the headline/CTA/service-chip overlay shared by all variants.
...
- No horizontal scroll anywhere on this surface — motion is vertical-only. Most sections use `components/ui/scroll-reveal.tsx` (IntersectionObserver → CSS reveal, no dependency); the desktop `Hero` and `SmoothScrollProvider` are the two places that use a real library (`gsap`/`ScrollTrigger`, `lenis`) because scroll-scrubbing a 120-frame canvas sequence and inertia scrolling aren't achievable with CSS alone — deliberate, scoped exceptions to the "no animation dependency" default elsewhere on this surface. (The mobile hero's crossfade, by contrast, is pure CSS — `.hero-dissolve`.)
