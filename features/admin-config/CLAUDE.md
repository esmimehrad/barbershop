...
`access-levels`, `staff-services`, `schedule`, `services`, `promotions`.
...
- `provider-picker.tsx` — **client** island used by service assignments; filters
  provider names without losing selections already made in the form.
...
- Tabs are server-first; client code is limited to form state and the searchable
  provider picker.
